...
import { ArrowLeft, Download, Edit3, FileText, Trash2, Wrench } from "lucide-react";
  ...
  loader: async ({ params }) => {
    try {
      const [item, cal] = await Promise.all([
        ...
        listEquipmentCalibrations(params.id),
      ]);
      return { item, history: cal.items };
    } catch (e) {
  ...
  notFoundComponent: () => (
    <AppShell title="Not found">
      <p className="text-sm text-muted-foreground">
        Equipment not found.{" "}
        <Link to="/equipment" className="text-[color:var(--color-primary)]">
          Back to list
        </Link>
      </p>
    </AppShell>
...
function EquipmentDetail() {
  ...
  const { item: loaded, history: loadedHistory } = Route.useLoaderData();
  const [item, setItem] = useState<AppEquipment>(loaded);
  const [editOpen, setEditOpen] = useState(false);
  const [saving, setSaving] = useState(false);

  const { data: liveHistory } = useQuery({
    queryKey: ["calibrations", item.id],
    queryFn: () => listEquipmentCalibrations(item.id),
    initialData: { items: loadedHistory, total: loadedHistory.length },
  });
  const history: AppCalibration[] = liveHistory?.items ?? loadedHistory;
  ...
  const onSave = async (payload: EquipmentPayload) => {
    setSaving(true);
    try {
      const updated = await updateEquipment(item.id, payload);
      setItem(updated);
      setEditOpen(false);
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      void router.navigate({ to: "/equipment" });
    } catch (e) {
  ...
  return (
    <AppShell
      breadcrumbs={[{ label: "Equipment", to: "/equipment" }, { label: item.tag || item.name }]}
    >
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" asChild>
            <Link to="/calibrations" search={{ tab: "log", equipment: item.id }}>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-xl border border-border bg-card shadow-xs">
            ...
            <ul className="divide-y divide-border">
              {history
                ...
                .map((c) => (
                  <li key={c.id} className="flex items-center gap-3 px-5 py-3 text-sm">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="font-medium text-foreground">{c.certificateNo}</div>
                      <div className="text-xs text-muted-foreground">
                        Issued {format(new Date(c.date), "MMM d, yyyy")} · {c.provider || "—"}
                      </div>
                    </div>
              ...
              {history.filter((c) => c.certificateNo).length === 0 && (
                <li className="px-5 py-6 text-center text-sm text-muted-foreground">
                  No certificate references yet.
                </li>
              )}
            </ul>
          </div>
        ...
        <aside className="space-y-6">
          <div className="rounded-xl border border-border bg-card p-5 shadow-xs">
            ...
            <div className="metric-value mt-1.5 text-2xl font-semibold text-foreground">
            ...
            >
              {days === null ? "No due date set" : days < 0 ? `${Math.abs(days)} days overdue` : `in ${days} days`}
            </div>
            ...
            <div className="mt-1.5 flex justify-between text-[11px] text-muted-foreground">
              <span>Last: {last ? format(last, "MMM d") : "—"}</span>
