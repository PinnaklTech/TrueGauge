...
export const Route = createFileRoute("/workspace/$slug/calibrations")({
...
function CalibrationsPage() {
  ...
  useEffect(() => {
    if (tab === "log" && me && !allowWrite) {
      setTab("schedules");
    }
  }, [tab, me, allowWrite]);

  useEffect(() => {
    const onTourCal = () => setTab("schedules");
    const onTourLog = () => setTab("log");
    const onTourCloseLog = () => setTab("schedules");
    window.addEventListener("tg-tour-open-calibrations", onTourCal);
    window.addEventListener("tg-tour-open-cal-log", onTourLog);
    window.addEventListener("tg-tour-close-cal-log", onTourCloseLog);
    return () => {
      window.removeEventListener("tg-tour-open-calibrations", onTourCal);
      window.removeEventListener("tg-tour-open-cal-log", onTourLog);
      window.removeEventListener("tg-tour-close-cal-log", onTourCloseLog);
    };
  }, []);
  ...
  return (
    <AppShell breadcrumbs={[{ label: "Calibration Schedules" }]} hidePageHeader>
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-1 rounded-lg border border-border bg-surface p-1" data-tour="cal-tabs">
          {(
            [
              { id: "schedules" as const, label: "Schedules & Urgency" },
              { id: "history" as const, label: `History Log (${calibrations.length})` },
              ...(allowWrite
                ? [{ id: "log" as const, label: "+ Log Calibration" }]
                : []),
            ]
          ).map((t) => (
            <button
              ...
              type="button"
              onClick={() => setTab(t.id)}
              data-tour={t.id === "log" ? "cal-log-tab" : undefined}
              className={cn(
      ...
      {tab === "schedules" && (
        <div className="space-y-6" data-tour="cal-schedules">
              ...
              <div className="grid gap-3 p-4 md:grid-cols-2">
                {overdue.map((e) => (
                  <UrgencyCard
                    ...
                    equipment={e}
                    tone="danger"
                    onLog={allowWrite ? () => setTab("log", e.id) : undefined}
                  />
                ))}
              </div>
            )}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {calLoading ? (
                  ...
                  <tr>
                    <td colSpan={5} className="p-4">
                      <PageState
                        ...
                        action={
                          allowWrite
                            ? { label: "Log calibration", onClick: () => setTab("log") }
                            : undefined
                        }
                  ...
                  calibrations.map((c) => (
                    <HistoryRow key={c.id} record={c} slug={slug} />
                  ))
                )}
              </tbody>
            </table>
...
function HistoryRow({ record: c, slug }: { record: AppCalibration; slug: string }) {
  return (
...
function LogCalibrationForm({
  ...
  useEffect(() => {
    if (!storageEnabled) setCertFile(null);
  }, [storageEnabled]);
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (certFile && storageEnabled) {
        try {
          ...
          // Calibration already saved — surface PDF failure without rolling back
          throw new Error(
            e instanceof Error
              ? `Calibration saved, but PDF upload failed: ${e.message}`
              : "Calibration saved, but PDF upload failed",
          );
        }
      }
      return cal;
    ...
    onSuccess: async () => {
      toast.success(
        certFile && storageEnabled
          ? "Calibration and PDF certificate saved"
  ...
  return (
    <div
      className="mx-auto max-w-2xl rounded-xl border border-border bg-card p-6 shadow-xs"
      data-tour="cal-log-form"
    >
      ...
      >
        ...
        <div className="space-y-1.5">
          ...
          <Textarea
            id="cal-log-notes"
            placeholder="Findings, adjustments, or remarks from this verification run…"
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            rows={4}
            maxLength={2000}
            className="min-h-24 resize-y bg-surface/40 leading-relaxed"
          />
          <p className="text-[11px] text-muted-foreground">
            Optional — saved on the calibration record and visible in history.
          </p>
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="cal-log-pdf">Certificate PDF (optional)</Label>
          {storageEnabled ? (
            <>
              <Input
                id="cal-log-pdf"
              ...
              />
              <p className="text-[11px] text-muted-foreground">
                PDF only · max 2 MB · uploaded after the calibration is saved
                {certFile ? ` · selected: ${certFile.name}` : ""}
              </p>
            </>
          ) : (
            <p className="text-[11px] text-muted-foreground">
              Certificate vault is not included in your plan. Contact TrueGage to attach PDFs.
            </p>
          )}
        </div>
